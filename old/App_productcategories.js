...
function ProductsView() {
  ...
  const loadProducts = async () => {
    try {
      const endpoint = selectedCategory ? `/products?category=${selectedCategory}` : '/products';
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded ${!selectedCategory ? 'bg-green-600 text-white' : 'bg-white'}`}
          >
            All
          </button>
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded ${selectedCategory === cat.id ? 'bg-green-600 text-white' : 'bg-white'}`}
            >
              {cat.name}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map(product => (
          <div key={product.id} className="bg-white rounded-lg shadow p-4">
            <div className="h-48 bg-gray-200 rounded mb-4 flex items-center justify-center text-6xl">
              {product.category_name === 'Fruits' ? ' ^=^m^n' : ' ^=  '}
            </div>
            <h3 className="font-bold text-lg mb-2">{product.name}</h3>
            <p className="text-gray-600 text-sm mb-2">{product.description}</p>
            <div className="flex items-center justify-between mb-4">
              <span className="text-xl font-bold text-green-600"> ^b {product.price}</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {categoryProducts.map(product => (
                      <div key={product.id} className="bg-white border-2 border-gray-200 rounded-xl hover:shadow-xl hover:border-green-400 transition-all">
                        <div className="p-5">
                          <div className="text-6xl mb-4 text-center">{product.image_url || ' ^=^s '}</div>
                          <h3 className="font-bold text-lg mb-2">{product.name}</h3>
                          <p className="text-sm text-gray-600 mb-4 line-clamp-2">{product.description}</p>
                          <div className="flex items-center justify-between mb-4">
                            <span className="text-2xl font-bold text-green-600"> ^b {product.price}</span>
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-center justify-between mb-4 pb-4 border-b">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-green-600"> ^b {product.price}</p>
                          </div>
                       ...
                       <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-2 bg-blue-50 text-blue-7>
                          >     <Edit size={16} />
                            Edit
                          </button>
                          <button
                            onClick={() => handleDelete(product.id)}
                            className="flex-1 flex items-center justify-center gap-2 bg-red-50 text-red-700>
                          >
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Price ( ^b ) *</label>
              <input
                type="number"
                step="0.01"
